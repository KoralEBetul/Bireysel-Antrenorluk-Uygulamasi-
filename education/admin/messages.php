...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      $warning_msg[] = 'Mesaj zaten silindi!';
   }
...
<section class="grid">
   ...
   <h1 class="dashboardheading">Mesajlar</h1>
   ...
   ?>
   <div class="box">
      <p>İsim Soyisim : <span><?= $fetch_messages['name']; ?></span></p>
      <p>Email : <span><?= $fetch_messages['email']; ?></span></p>
      <p>Telefon : <span><?= $fetch_messages['number']; ?></span></p>
      <p>Mesajı : <span><?= $fetch_messages['message']; ?></span></p>
      <form action="" method="POST">
         <input type="hidden" name="delete_id" value="<?= $fetch_messages['id']; ?>">
         <input type="submit" value="Mesajı sil" onclick="return confirm('Mesajı silmek istediğine emin misin?');" name="delete" class="adminbtn">
      </form>
   </div>
   <?php
      }
